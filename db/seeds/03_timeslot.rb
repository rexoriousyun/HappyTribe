timeslot_1 = Timeslot.create(
  start_time: Time.new(2016, 5, 30, 9, 0, 0),
  end_time: Time.new(2016, 5, 30, 12, 0, 0),
  capacity: 5,
  event_id: 1
  );
timeslot_2 = Timeslot.create(
  start_time: Time.new(2016, 7, 29, 12, 30, 0),
  end_time: Time.new(2016, 7, 29, 15, 0, 0),
  capacity: 10,
  event_id: 1
  );
timeslot_3 = Timeslot.create(
  start_time: Time.new(2016, 8, 29, 15, 0, 0),
  end_time: Time.new(2016, 8, 29, 18, 0, 0),
  capacity: 4,
  event_id: 2
  );
timeslot_4 = Timeslot.create(
  start_time: Time.new(2016, 9, 30, 9, 0, 0),
  end_time: Time.new(2016, 9, 30, 12, 0, 0),
  capacity: 2,
  event_id: 2
  );
timeslot_5 = Timeslot.create(
  start_time: Time.new(2016, 10, 30, 12, 30, 0),
  end_time: Time.new(2016, 10, 30, 15, 0, 0),
  capacity: 3,
  event_id: 3
  );
timeslot_6 = Timeslot.create(
  start_time: Time.new(2016, 11, 30, 15, 0, 0),
  end_time: Time.new(2016, 11, 30, 18, 0, 0),
  capacity: 5,
  event_id: 3
  );
timeslot_7 = Timeslot.create(
  start_time: Time.new(2016, 12, 30, 9, 0, 0),
  end_time: Time.new(2016, 12, 30, 12, 0, 0),
  capacity: 2,
  event_id: 4
  );
timeslot_8 = Timeslot.create(
  start_time: Time.new(2016, 3, 15, 12, 30, 0),
  end_time: Time.new(2016, 3, 15, 15, 0, 0),
  capacity: 3,
  event_id: 4
  );
timeslot_9 = Timeslot.create(
  start_time: Time.new(2016, 3, 15, 15, 0, 0) ,
  end_time: Time.new(2016, 3, 15, 18, 0, 0),
  capacity: 1,
  event_id: 5
  );
timeslot_10 = Timeslot.create(
  start_time: Time.new(2016, 3, 25, 15, 0, 0),
  end_time: Time.new(2016, 3, 25, 18, 0, 0),
  capacity: 5,
  event_id: 6
  );
timeslot_11 = Timeslot.create(
  start_time: Time.new(2016, 3, 25, 17, 0, 0),
  end_time: Time.new(2016, 3, 25, 20, 0, 0),
  capacity: 1,
  event_id: 6
  );
timeslot_12 = Timeslot.create(
  start_time: Time.new(2016, 3, 22, 8, 0, 0) ,
  end_time: Time.new(2016, 3, 22, 11, 0, 0),
  capacity: 5,
  event_id: 7
  );
timeslot_13 = Timeslot.create(
  start_time: Time.new(2016, 2, 1, 11, 0, 0) ,
  end_time: Time.new(2016, 2, 1, 14, 0, 0),
  capacity: 1,
  event_id: 7
  );
timeslot_14 = Timeslot.create(
  start_time: Time.new(2016, 2, 1, 15, 0, 0) ,
  end_time: Time.new(2016, 2, 1, 18, 0, 0),
  capacity: 5,
  event_id: 8
  );
timeslot_15 = Timeslot.create(
  start_time: Time.new(2016, 12, 25, 8, 0 ,0),
  end_time: Time.new(2016, 12 ,25, 14, 0, 0),
  capacity: 2,
  event_id: 8
  );
timeslot_16 = Timeslot.create(
  start_time: Time.new(2016, 12, 25, 14, 0 ,0),
  end_time: Time.new(2016, 12 ,25, 20, 0, 0),
  capacity: 2,
  event_id: 9
  );
timeslot_17 = Timeslot.create(
  start_time: Time.new(2016, 11, 19, 16, 0 ,0),
  end_time: Time.new(2016, 11, 19, 19, 0, 0),
  capacity: 4,
  event_id: 9
  );